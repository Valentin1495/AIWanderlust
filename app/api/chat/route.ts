...
export async function POST(req: Request) {
  ...
  const firstRes = await fetch(`${API_URL}?key=${API_KEY}`, {
    method: 'POST',
    headers: {
  ...
  const sights = firstData.candidates[0].content;
  ...
  const secondRes = await fetch(`${API_URL}?key=${API_KEY}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const plan = secondData.candidates[0].content;
